...
export default function HomePage() {
  ...
  const limitedTransactions =
    dashData?.transactionHistory?.slice(0, 5) ?? [];
      ...
      <ScrollView
        ...
        <HomeWalletCard
          ...
          accountNumber={
            isLoading
              ? '----------'
              : dashData?.accountNumber ?? '----------'
          }
          accountTier={
            isLoading ? 'Tier -- Account' : accountTier
          }
          upgradeLabel={isLoading ? null : upgradeLabel}
          onUpgradePress={
            upgradeLabel
              ? () => navigateTo('upgradeAccount')
              : null
          }
        />
          ...
          <View className="flex-row justify-between">
            {goodies.map((goodie, index) => (
              <TouchableOpacity
                ...
                <Ionicons
                  name={index === 0 ? 'gift' : 'cash'}
                  size={24}
                  color="white"
                />

                <Text className="mb-1 mt-2 text-xs text-white">
                  {goodie.title}
                </Text>

                <Text className="text-lg font-bold text-white">
                  {goodie.points || goodie.amount}
                </Text>
        ...
        <View className="px-5 pb-6">
          <View className="mb-3 flex-row items-center justify-between">
            ...
            <TouchableOpacity
              onPress={() => navigateTo('allTransactions')}>
              <Text className="text-sm font-medium text-[#0E7490]">
                View All
              </Text>
            </TouchableOpacity>
          </View>
