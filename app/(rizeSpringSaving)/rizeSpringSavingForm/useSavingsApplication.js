...
export function useSavingsApplication() {
    ...
    // ⚠️ Filtered to FIXED type only — this form is for Fixed Savings
    const fixedProducts = savingProducts.filter((p) => p.savingProductType === 'FIXED');

    const productOptions = fixedProducts.map((p) => ({
        label: p.productName,
        value: String(p.id),
    }));
    ...
    const handleProductSelect = (value) => {
        const product = fixedProducts.find((p) => String(p.id) === value);
    ...
    const handleBookSaving = async () => {
        if (!selectedProduct || !amount || !description || !transactionMode || !tenorDuration || !savingAccountNumber) {
            Toast.show({
                type: 'error',
        ...
        const payload = {
            ...
            description,
            interestPeriod: `${tenorDuration} ${getTenorUnit(selectedProduct.tenorDisplayIn, parseInt(tenorDuration))}`,
